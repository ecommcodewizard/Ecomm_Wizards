...
export default function SplusFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="spd-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="spd-faq__item">
            <button
            ...
            >
              <span className="spd-faq__q">{faq.q}</span>
              <span
                className="spd-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="spd-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
