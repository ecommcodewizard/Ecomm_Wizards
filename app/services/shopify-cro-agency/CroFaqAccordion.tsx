...
export default function CroFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="scro-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="scro-faq__item">
            <button
            ...
            >
              <span className="scro-faq__q">{faq.q}</span>
              <span
                className="scro-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="scro-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
