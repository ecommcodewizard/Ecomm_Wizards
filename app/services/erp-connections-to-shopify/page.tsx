...
const INTRO_ROWS = [
  ...
  },
  {
    ...
    img: "/images/video_cover_2048x.webp",
    imageLeft: false,
  },
  {
  ...
  return (
    <>
                ...
                <div className="erp-acc-list">
                  {SYNC_ITEMS.map((it, i) => (
                    <div key={i} className="erp-acc-item" data-acc={`erp-sync-${i}`}>
                      ...
                      <div className="erp-acc-bodyc"><div className="erp-acc-in"><p>{it.a}</p></div></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="erp-faq-list">
              {FAQS.map((f, i) => (
                <div key={i} className="erp-faq-item" data-faq={`erp-faq-${i}`}>
                  ...
                  <div className="erp-faq-bodyc"><div className="erp-faq-in"><p>{f.a}</p></div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .erp-acc-bodyc  { display: grid; grid-template-rows: 0fr; opacity: 0; transition: grid-template-rows .3s ease, opacity .25s ease; }
        .erp-acc-in { overflow: hidden; min-height: 0; }
        .erp-acc-bodyc p { color: #334155; font-size: 15px; line-height: 1.75; margin: 0; padding: 0 24px 22px 0; }
        ${["erp-sync", "erp-adv"]
          .flatMap((g) =>
        ...
        .erp-faq-bodyc  { display: grid; grid-template-rows: 0fr; opacity: 0; transition: grid-template-rows .3s ease, opacity .25s ease; }
        .erp-faq-in { overflow: hidden; min-height: 0; }
        .erp-faq-bodyc p { color: #334155; font-size: 15px; line-height: 1.75; margin: 0; padding: 0 60px 24px 0; }
