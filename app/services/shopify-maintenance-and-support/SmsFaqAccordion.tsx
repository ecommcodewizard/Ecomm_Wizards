...
export default function SmsFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="sms-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="sms-faq__item">
            <button
            ...
            >
              <span className="sms-faq__q">{faq.q}</span>
              <span
                className="sms-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="sms-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
