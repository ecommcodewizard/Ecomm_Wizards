...
export default function SatFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="sat-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="sat-faq__item">
            <button
            ...
            >
              <span className="sat-faq__q">{faq.q}</span>
              <span
                className="sat-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="sat-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
