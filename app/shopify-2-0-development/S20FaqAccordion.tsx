...
export default function S20FaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="s20-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="s20-faq__item">
            <button
            ...
            >
              <span className="s20-faq__q">{faq.q}</span>
              <span
                className="s20-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="s20-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
