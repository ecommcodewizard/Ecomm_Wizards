...
export default function BookFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="bkc-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="bkc-faq__item">
            <button
            ...
            >
              <span className="bkc-faq__q">{faq.q}</span>
              <span
                className="bkc-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="bkc-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
