...
export default function SiFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="si-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="si-faq__item">
            <button
            ...
            >
              <span className="si-faq__q">{faq.q}</span>
              <span
                className="si-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="si-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
