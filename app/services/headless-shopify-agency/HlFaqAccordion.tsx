...
export default function HlFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="shl-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="shl-faq__item">
            <button
            ...
            >
              <span className="shl-faq__q">{faq.q}</span>
              <span
                className="shl-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="shl-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
