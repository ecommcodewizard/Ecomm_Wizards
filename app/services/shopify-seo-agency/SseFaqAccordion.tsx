...
export default function SseFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="sse-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="sse-faq__item">
            <button
            ...
            >
              <span className="sse-faq__q">{faq.q}</span>
              <span
                className="sse-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="sse-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
