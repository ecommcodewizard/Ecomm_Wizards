...
export default function SdaFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="sda-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="sda-faq__item">
            <button
            ...
            >
              <span className="sda-faq__q">{faq.q}</span>
              <span
                className="sda-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="sda-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
