...
export default function AuditFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="aud-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="aud-faq__item">
            <button
            ...
            >
              <span className="aud-faq__q">{faq.q}</span>
              <span
                className="aud-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="aud-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
