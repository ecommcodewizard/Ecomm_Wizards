...
export default function SaoFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="sao-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="sao-faq__item">
            <button
            ...
            >
              <span className="sao-faq__q">{faq.q}</span>
              <span
                className="sao-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="sao-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
