...
export default function SsdFaqAccordion({ faqs }: { faqs: Faq[] }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <ul className="ssd-faq__list">
      {faqs.map((faq, i) => {
        const isOpen = openIndex === i;
        return (
          <li key={faq.q} className="ssd-faq__item">
            <button
            ...
            >
              <span className="ssd-faq__q">{faq.q}</span>
              <span
                className="ssd-faq__chev"
                aria-hidden
            ...
            >
              <div style={{ overflow: "hidden" }}>
                <p className="ssd-faq__a-text">{faq.a}</p>
              </div>
            </div>
          </li>
        );
